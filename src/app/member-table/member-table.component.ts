...
export class MemberTableComponent implements OnInit, AfterViewInit {
  ...
  allShortlistPreawardData: Company[] = [
    {
      name: "Jeny pvt ltd",
      members: {
  ...
  onCommitteeChange(index: number) {
    console.log('Selected committee:', this.committees[index]);
    // Implement logic to update displayed data based on selected committee
  }
  ...
  onResponseFilterChange(filter: string) {
    ...
    switch (filter) {
      case 'all-shortlist':
        this.selectedData = this.allShortlistData;
        break;
      case 'all-preaward':
        this.selectedData = this.allPreawardData;
        break;
      case 'all-shortlist-preaward':
        this.selectedData = this.allShortlistPreawardData;
        break;
    }
  }
